...
function snp_popup_submit()
{
    ...
    if (count($errors) > 0) {
        ...
        if (isset($POPUP_META['snp_contact_properties'][0])) {
            $contactPropertiesArray = unserialize($POPUP_META['snp_contact_properties'][0]);

            for ($i = 0; $i <= count($contactPropertiesArray['key']);  $i++) {
                if (empty($contactPropertiesArray['key'][$i])) {
                    continue;
                }

                $contactProperties[$contactPropertiesArray['key'][$i]] = $contactPropertiesArray['value'][$i];
            }
        }
