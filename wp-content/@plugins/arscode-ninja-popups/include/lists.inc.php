...
function snp_ml_get_gr_lists($ml_gr_apikey='')
{
    ...
    try {
        $response = \Httpful\Request::get('https://api.getresponse.com/v3/campaigns')
            ->expectsJson()
            ->addHeader('X-Auth-Token',  'api-key ' . $ml_gr_apikey)
            ->send();

        foreach($response->body as $it) {
            $list[$it->campaignId] = array('name' => $it->name);
        }
    } catch (Expcetion $e) {
        $list[0] = array('name' => 'Connection problem');
    }
...
function snp_ml_get_gr_fields($ml_gr_apikey='')
{
    if (!$ml_gr_apikey) {
        $ml_gr_apikey = snp_get_option('ml_gr_apikey');
    }
    ...
    try {
        $response = \Httpful\Request::get('https://api.getresponse.com/v3/custom-fields')
            ->expectsJson()
            ->addHeader('X-Auth-Token',  'api-key ' . $ml_gr_apikey)
            ->send();

        foreach($response->body as $it) {
            $list[$it->customFieldId] = array('name' => $it->name);
        }
    } catch (Expcetion $e) {
        $list[0] = array('name' => 'Connection problem');
    }
